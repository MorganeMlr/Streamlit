...
if page == pages[6] : 
    ...
    st.write("<h1><span style='color: #27DCE0;'>- La géographie</span> influence les inégalités de salaire dans la mesure où il est possible de regrouper les départements métropolitains et les villes dans trois groupes bien définis répartis inégalement sur le territoire.</h5>", unsafe_allow_html=True)
    st.write("- Les facteurs qui influencent principalement ce regroupement sont <h1><span style='color: #27DCE0;'>la dimension urbaine ou rurale de ces zones</span>, qui elle-même influence <h5><span style='color: #27DCE0;'>le dynamisme économique </span> et donc la richesse de ces localités. On remarque notamment la position très spécifique de Paris et de sa proche banlieue par rapport au reste de la France.</h1>", unsafe_allow_html=True)
