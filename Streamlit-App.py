...
if page == pages[3] : 
    ...
    if option == 'La matrice de corrélation':
       # Afficher l'image 
       image_path = "Images/Matrice_de_correlation.png"  
...
if page == pages[6] : 
    ...
    st.write("<p><span style='color: #27DCE0;'>- La géographie</span> influence les inégalités de salaire dans la mesure où il est possible de regrouper les départements métropolitains et les villes dans trois groupes bien définis répartis inégalement sur le territoire.</h5>", unsafe_allow_html=True)
    st.write("- Les facteurs qui influencent principalement ce regroupement sont <p><span style='color: #27DCE0;'>la dimension urbaine ou rurale de ces zones</span>, qui elle-même influence <p>><span style='color: #27DCE0;'>le dynamisme économique </span> et donc la richesse de ces localités. On remarque notamment la position très spécifique de Paris et de sa proche banlieue par rapport au reste de la France.</h1>", unsafe_allow_html=True)
    st.write("- Un point à prendre en compte est que nous nous sommes intéressés aux <p><span style='color: #27DCE0;'>inégalités de salaire sur le territoire français</span>, mais pas aux inégalités de niveau de vie. En effet, prendre en compte un salaire horaire sans tenir compte du coût de la vie à un endroit spécifique est une démarche intéressante, mais incomplète.")
